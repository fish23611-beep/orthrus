...
# C2: Import wandb_control for mode-aware W&B initialization
try:
    from wandb_control import resolve_wandb_mode, init_wandb, wandb_log, wandb_finish
except ImportError:
    # Fallback if wandb_control not available
    resolve_wandb_mode = None
    init_wandb = None
    wandb_log = None
    wandb_finish = None
...
if __name__ == '__main__':
    ...
    # C2: Use mode-aware W&B initialization
    if init_wandb is not None and resolve_wandb_mode is not None:
        wandb_mode = resolve_wandb_mode(cfg, args)
        run = init_wandb(cfg, args, wandb_mode)
        wandb.config.update(remove_underscore_keys(dict(cfg), keys_to_keep=["_task_path"]))
    else:
        # Fallback to original behavior
        wandb.init(
            mode="online" if args.wandb else "disabled",
            project="orthrus_repo",
            name=exp_name,
            tags=tags,
        )
        wandb.config.update(remove_underscore_keys(dict(cfg), keys_to_keep=["_task_path"]))
    ...
    time_consumption = main(cfg, args)

    # C2: Use mode-aware W&B logging
    if wandb_finish is not None:
