...
- All paths use strings for YACS compatibility.
...
def resolve_artifact_root(
    ...
    """
    Resolve the canonical artifact-root path as a string. No filesystem side effects.
...
def resolve_run_dir(
    ...
    ----------
    artifact_root:
        Resolved artifact root string.
    dataset:
        Non-empty str dataset name.
    model_variant:
        Non-empty str model variant name.
    seed:
        Integer seed.
    ...
    -------
    str
        Run directory path as string. No directories are created.
    """
    ...
    return str(Path(artifact_root).joinpath(*parts))
...
_STAGE_DIRS: dict[str, list[str]] = {
    "train":     ["checkpoints"],
    "test":      ["edge_scores"],
    "evaluate":  ["node_scores"],
}
...
def create_stage_directories(
    ...
    ----------
    run_dir:
        The canonical run directory string.
    stages:
    ...
    for stage in stages:
        ...
        if subs is None:
            continue
        for sub in subs:
            to_create.add(str(Path(run_dir) / sub))
...
def resolve_artifact_paths(
    ...
    *,
    ...
    # 2. Resolve run dir (strict inputs; raises ValueError for invalid inputs)
    if run_dir is None:
    ...
    cfg.detection.gnn_training._trained_models_dir = str(Path(run_dir) / "checkpoints")
    cfg.detection.gnn_testing._edge_losses_dir     = str(Path(run_dir) / "edge_scores")
    cfg.detection.evaluation.node_evaluation._precision_recall_dir = str(Path(run_dir) / "node_scores")
