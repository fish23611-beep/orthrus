...
class TestParseStages:
    ...
    def test_none_no_run_from_training_full(self):
        """The default pipeline excludes tracing unless explicitly requested."""
        result = _parse_stages(None, run_from_training=False)
        assert result == ["preprocess", "train", "test", "evaluate"]
    ...
    def test_no_stages_disables_trace_by_default(self):
        """Default (no --stages, no --run_from_training) excludes trace."""
        result = _parse_stages(None, run_from_training=False)
...
class TestTraceGating:
    ...
    @patch('orthrus.build_feature_word2vec')
    @patch('orthrus.embed_edges_feature_word2vec')
    def test_default_no_stages_no_run_from_training_excludes_trace(
            self, m_embed, m_w2v, m_graphs,
            m_tracing, m_eval, m_test, m_train, m_wandb,
            m_check, mock_cfg, mock_args):
        """Default pipeline runs detection/evaluation but leaves tracing off."""
        import orthrus
